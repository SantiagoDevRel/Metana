...
describe("External Return Contract", function () {
    let contract = null, deployer, user2;
    ...
    it("Balance owner should be 10", async () => {
        ...
        expect(await contract.getBalance(deployer.address)).to.be.equal(new BigNumber.from(10))
    })

    it("User 2 balance should be 0", async () => {
        expect(await contract.getBalance(user2.address)).to.be.equal(new BigNumber.from(0))
    })

    it("User 2 balance should be 5", async () => {
        const tx = await contract.connect(deployer).transfer(deployer.address, user2.address, 5);
        await tx.wait()
        expect(await contract.getBalance(user2.address)).to.be.equal(new BigNumber.from(5))
    })

    it("Test TX signature transfer", async () => {
        //CHECK the return value of the function transfer() - but NOT actually executing it.
        const tx = await contract.callStatic.transfer(deployer.address, user2.address, 10);
