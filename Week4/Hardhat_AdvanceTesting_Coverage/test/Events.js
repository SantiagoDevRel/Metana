...
describe("Test Events contract", () => {
    let contract = null, deployerSigner = null, user2Signer = null;
    ...
    it("Should emit first event", async () => {
        ...
        expect(await contract.connect(deployerSigner).firstEvent()).to.emit(contract, "First").withArgs(deployerSigner.address)
    })

    it("Should emit second event", async () => {
        expect(await contract.connect(user2Signer).secondEvent()).to.emit(contract, "Second").withArgs()
