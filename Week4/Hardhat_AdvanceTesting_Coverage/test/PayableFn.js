...
describe("Testing Payable functions", () => {
    let contract = null, deployer = null, user2 = null, provider = null;
    ...
    beforeEach(async () => {
        ...
        */
        const amountToSend = ethers.utils.hexStripZeros(ethers.utils.parseEther("200").toHexString())
    ...
    it("Deployer should have 200 ethers", async () => {
        ...
        expect(balance).to.be.equal(new BigNumber.from(ethers.utils.parseEther("200")))
    })

    it("Deposit 100 ethers in the contract from deployer", async () => {
        const tx = await contract.connect(deployer).deposit({ value: ethers.utils.parseEther("100") })
        await tx.wait()
        ...
        */
        ...
        expect(balanceDeployer).to.be.closeTo(
            ...
            new BigNumber.from(ethers.utils.parseEther("0.001"))) //<-- range expected


        const balanceContract = await provider.getBalance(contract.address);
        expect(balanceContract).to.be.equal(new BigNumber.from(ethers.utils.parseEther("100")))
    })

    it("Withdraw 100 ethers from deployer after deposit", async () => {
        const tx1 = await contract.connect(deployer).deposit({ value: ethers.utils.parseEther("100") })
        await tx1.wait()
    ...
    it("Withdraw 100 ethers with no deposit (revert)", async () => {
        ...
        await expect(contract.connect(deployer).withdraw()).to.be.revertedWith("You don't have any ether here.")
    })
