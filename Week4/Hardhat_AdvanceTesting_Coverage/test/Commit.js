...
describe("External Return Contract", function () {
  ...
  it("Deployer try to bet 2 ether (reverted - must bet 1 ether)", async () => {
    await expect(
      contract
        .connect(deployer)
        .gambleOnTheBlockNumber({ value: ethers.utils.parseEther("2") })
    ).to.be.revertedWith("Commit: Please pay 1 ether");
  });

  it("Deployer try to claimWinning without betting (reverted)", async () => {
    await expect(contract.connect(deployer).claimWinning()).to.be.revertedWith(
      "Commit: Too late to claim your rewards"
    );
  });
  ...
  it("Deployer bet 1 ether and wait 50 blocks to claim", async () => {
    /*
        ...
        */
    const mint101Blocks = ethers.utils.hexStripZeros(
      ethers.utils.hexValue(101)
    );
    ...
    const tx = await contract
      .connect(deployer)
      .gambleOnTheBlockNumber({ value: ethers.utils.parseEther("1") });
    await tx.wait();
  ...
  it("Deployer comes too late to claim rewards(after 100 blocks)", async () => {
    ...
    const mint200Blocks = ethers.utils.hexStripZeros(
