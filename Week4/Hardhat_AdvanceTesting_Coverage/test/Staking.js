...
describe("Test Staking contract", () => {
  ...
  it("Deployer should have 100 ethers", async () => {
    ...
    expect(balanceDeployer).to.be.equal(
  ...
  it("Stake 30 ethers", async () => {
    const tx = await contract
      .connect(deployer)
      .deposit({ value: ethers.utils.parseEther("30") });
    await tx.wait();
    ...
    expect(amountStaked).to.be.equal(
      new BigNumber.from(ethers.utils.parseEther("30"))
    );
  });

  it("Try to withdraw with no previous stake (reverted)", async () => {
    expect(contract.connect(deployer).withdraw()).to.be.revertedWith(
      "Staking: You don't have funds in this contract"
    );
  });
  ...
  it("Stake 50 ethers and try to withdraw rigth away (reverted)", async () => {
    ...
    const { amountStaked } = await contract.staked(deployer.address);
    expect(amountStaked).to.be.equal(
      new BigNumber.from(ethers.utils.parseEther("50"))
    );

    await expect(contract.connect(deployer).withdraw()).to.be.revertedWith(
      "Staking: Please wait at least 10 seconds to withdraw"
    );
  });
  ...
  it("Stake 70 ethers and try to withdraw 20 seconds later", async () => {
    ...
    const tx = await contract
      .connect(deployer)
      .deposit({ value: ethers.utils.parseEther("70") });
    await tx.wait();
