...
describe("Test unlock accounts", () => {
    let accounts = null, dai = null, binance = null, myAccount = null;
    ...
    it("Unlock account ", async () => {
        ...
        expect(balanceOfBinance).to.be.greaterThan(new BigNumber.from(ethers.utils.parseEther("10000000")))
    })
    ...
    it("Expect 1000 DAI in my account from Binance account", async () => {
        ...
        const tx = await dai.connect(binance).transfer(myAccountAddress, amountOfDai)
        await tx.wait()

        //expect my balance to be 1000 DAI
        expect(await dai.balanceOf(myAccountAddress)).to.be.equal(new BigNumber.from(amountOfDai))
    })
