...
const ApproveSignature = "0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925";
  ...
  return (
    <>
      <div className={styles.containerButtons}>
        <button className={styles.button} onClick={() => formatLogs(`${TransferSignature}`)}>
          Update Transfer Logs
        </button>
        <div className={styles.title}>
          From the block #{startingBlock} <br></br> to the block #{currentBlock}
        </div>
        <button className={styles.button} onClick={() => formatLogs(`${ApproveSignature}`)}>
          ...
          <ol className={styles.logs}>
            {transactionsArray.map((tx) => (
              <li className={styles.everyLog} key={tx.id}>
                ...
                <div>To: {tx.to}</div>
                <div>
                  <a className={styles.a} href={tx.txHash} target="_blank" rel="noreferrer">
                    View in Etherscan <img className={styles.img} alt="Etherscan Logo" src="https://etherscan.io/images/brandassets/etherscan-logo-circle-light.svg" />
                  </a>
                </div>
              </li>
            ))}
